...
def trim_cross_comparison(df1, df2, num_insts, num_classes):
    ...
    df1_sampled.index = list(range(len(df1_sampled.index)))
    df2_sampled = df2_sampled.sort_values('class_label')
    df2_sampled.index = list(range(len(df2_sampled.index)))
...
def trim_sample_df(df, num_insts, classes):
    ...
    print("step 2")
    print(df)
    df = sample_classes(df, list(classes))
    print("step 3")
    print(df)
    # restart index
...
def ngrams_bursts(len_seq, round=None):
    ...
    ngrams = np.array(list(map(sum, bursts)))
    ...
    with open(fpath) as f:
        ...
        try:
            for pcap_filename, values in data_dict.items():
                ...
                order = values['order']
                lengths = recover_order(*list(map(np.array, [snd, rcv, order])))
